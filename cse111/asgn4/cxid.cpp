...
void reply_put(accepted_socket& client_sock, cxi_header& header) {
   ...
   if (file_stream) {
      char* put_output = new char[header.nbytes+1];
      recv_packet (client_sock, put_output, header.nbytes);
      outlog << "received header " << header << endl;

      put_output[header.nbytes] = '\0';
      file_stream.write(put_output, header.nbytes);
...
void reply_get(accepted_socket& client_sock, cxi_header& header) {
   ifstream file_stream(header.filename, ofstream::binary);
   if (file_stream) {
...
void reply_rm(accepted_socket& client_sock, cxi_header& header) {
   if(unlink(header.filename)) {
      ...
      header.nbytes = 0;
      memset(header.filename, 0, FILENAME_SIZE);
      outlog << "cxid_rm: unlink success: ";
   }else {
      header.command = cxi_command::ACK;
      header.nbytes = 0;
      memset(header.filename, 0, FILENAME_SIZE);
      outlog << "cxid_rm: unlink error: ";
   }
   outlog << "sending header " << header << endl;
   send_packet(client_sock, &header, sizeof header);
}
